...
Elm.Main.make = function (_elm) {
   ...
   shots) {
      return A2($List.map,
      $Shot.applyPhysics(dt),
      shots);
   });
   var updateEnemies = F2(function (dt,
   enemies) {
      return A2($List.map,
...
Elm.Ship.make = function (_elm) {
   ...
   $Basics = Elm.Basics.make(_elm),
   $Util = Elm.Util.make(_elm);
   var updateShooting = F2(function (isShooting,
   ship) {
      return _U.replace([["shooting"
                         ,isShooting]],
      ship);
   });
   var updateVel = F2(function (newVel,
   ship) {
      return _U.replace([["vel"
                         ,newVel]],
      ship);
   });
   var applyPhysics = F2(function (dt,
   ship) {
      return function () {
         var v = ship.vel;
         var p = ship.pos;
...
Elm.Shot = Elm.Shot || {};
Elm.Shot.make = function (_elm) {
   "use strict";
   _elm.Shot = _elm.Shot || {};
   if (_elm.Shot.values)
   return _elm.Shot.values;
   var _op = {},
   _N = Elm.Native,
   _U = _N.Utils.make(_elm),
   _L = _N.List.make(_elm),
   ...
   var initShot = {_: {}
                  ,pos: $Util.zeroVec2
                  ,vel: $Util.zeroVec2};
   var Shot = F2(function (a,b) {
      return {_: {},pos: a,vel: b};
   });
   _elm.Shot.values = {_op: _op
                      ,Shot: Shot
                      ,initShot: initShot
                      ,applyPhysics: applyPhysics};
   return _elm.Shot.values;
...
Elm.Util.make = function (_elm) {
   ...
   $moduleName = "Util",
   $Basics = Elm.Basics.make(_elm);
   var Rectangle = F2(function (a,
   b) {
      return {_: {}
             ,center: a
             ,size: b};
   });
   var dotProduct = F2(function (a,
   b) {
      return a.x * b.x + a.y * b.y;
   });
   var vecAdd = F2(function (a,b) {
      return {_: {}
             ,x: a.x + b.x
             ,y: a.y + b.y};
   });
   var vecScalarMul = F2(function (v,
   c) {
      return {_: {}
             ,x: v.x * c
             ,y: v.y * c};
   });
   var vecSubtract = F2(function (a,
   b) {
      return A2(vecAdd,
      a,
      A2(vecScalarMul,b,-1));
   });
   var overlap = F2(function (a,
   b) {
      return function () {
         var bSizeHalf = A2(vecScalarMul,
         b.size,
         0.5);
         var aSizeHalf = A2(vecScalarMul,
         a.size,
         0.5);
         var maxDx = aSizeHalf.x + bSizeHalf.x;
         var maxDy = aSizeHalf.y + bSizeHalf.y;
         var sub = A2(vecSubtract,
         a.center,
         b.center);
         return _U.cmp($Basics.abs(sub.x),
         maxDx) < 0 && _U.cmp($Basics.abs(sub.y),
         maxDy) < 0;
      }();
   });
   var zeroVec2 = {_: {}
